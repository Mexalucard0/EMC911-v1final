		...
		if result[1] == nil then
			if xPlayer.getMoney() >= Config.InitialLockerRentPrice then
				...
				TriggerClientEvent('tnotify:client:SendTextAlert', _source, {
					style  =  'success',
					duration  =  5500,
					message = '你即将租用' ..lockerName.. '. 价格为$'..Config.DailyRentPrice..'每日.',
					sound  =  true
				})
			else
				TriggerClientEvent('tnotify:client:SendTextAlert', _source, {
	...
	MySQL.Async.fetchAll('SELECT * FROM inventory_lockers', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer.source, { type = 'inform', text = '你支付了$'..Config.DailyLockerRentPrice..'以租用储物柜.', length = 8000 })
			else
				if oldESX then
