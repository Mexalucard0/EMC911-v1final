...
RegisterNetEvent('fivem_vehicleshop:ShowNotifyESX')
AddEventHandler('fivem_vehicleshop:ShowNotifyESX', function(msg)
	ShowNotifyESX(msg)
end)
...
function ProduceNumberPlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('fivem_vehicleshop:PlateInUse', function (PlateInUse)
	...
	if option == "repay" then
		if plate ~= nil or not plate == '' then
			if amount ~= nil or amount >= 1 then
				ESX.TriggerServerCallback('fivem_vehicleshop:GetOwnedVehByPlate', function(vehPlate, vehPrice, vehHash, vehFinance, vehRepaytime)
...
RegisterCommand('registration', function(source, args)
	...
	if option == "view" then
		TriggerServerEvent('fivem_vehicleshop:openRegSV', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), plate)
	
	-- Show Registration Paper:
	elseif option == "show" then
		local player, distance = ESX.Game.GetClosestPlayer()
		if distance ~= -1 and distance <= 2.0 then
			TriggerServerEvent('fivem_vehicleshop:openRegSV', GetPlayerServerId(PlayerId()), GetPlayerServerId(player), plate)
		else
	...
	elseif option == "give" then
		local player, distance = ESX.Game.GetClosestPlayer()
		if distance ~= -1 and distance <= 2.0 then
			TriggerServerEvent('fivem_vehicleshop:GiveRegistrationPaper', GetPlayerServerId(PlayerId()), GetPlayerServerId(player), plate)
		else
			ShowNotifyESX(_U('nobody_near'))
		end
...
function DrawTextOptions(carPos,currentName,currentDownpayment,currentCommission,swapCar,currentPrice,currentStock)
	if (PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel) then
		-- info
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.6, currentName.."~s~ | ~y~$"..comma_value(math.floor(currentPrice + (currentPrice * (currentCommission / 100)))).."~s~| ~m~"..currentCommission.."%~s~ Commission ~g~[Z]~s~ | ~b~"..currentStock.."~s~ PCS")
		-- buttons:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.54, "~g~[H]~s~ Test-Drive | "..swapCar)
		-- finance:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.48, "~g~[K]~s~ Finance | ~m~"..currentDownpayment.."%~s~ Downpayment ~g~[X]~s~ | ~y~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * (currentDownpayment/100))).."~s~")
		-- cost display:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.42, "~r~"..Config.InterestRate.."%~s~ Interest Rate | ~y~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * ((Config.InterestRate/100)+1))).."~s~ Total Cost | ~g~[E]~s~ Buy")
	else
		-- info
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.6, currentName.."~s~ | ~y~$"..comma_value(math.floor(currentPrice + (currentPrice * (currentCommission / 100)))).." ~s~ "..currentStock.."~s~ PCS")
		-- buttons:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.54, "~m~"..currentCommission.."%~s~ Commission~s~")
		-- finance:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.48, "~g~[K]~s~ Finance | ~m~"..currentDownpayment.."%~s~ Downpayment | ~y~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * (currentDownpayment/100))).."~s~")
		-- cost display:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.42, "~r~"..Config.InterestRate.."%~s~ Interest Rate | ~g~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * ((Config.InterestRate/100)+1))).."~s~ Total Cost | ~g~[E]~s~ Buy")
	end
end
