  ...
  while true do
    if not robbery.inHouse then
      ...
      if closestDist < Config.ActDist then
        if robbery.cops and robbery.cops >= Config.MinCopsOnline and not robbery.tryHouse then
          if Config.UseDraw3D then
            ESX.Game.Utils.DrawText3D(closest, "Press [~r~E~s~] to break in.",1,7)
          else
            ESX.ShowHelpNotification("Press ~INPUT_PICKUP~ to break in.")
          end
      ...
      if exitDist and exitDist < Config.ActDist then
        if Config.UseDraw3D then
          ESX.Game.Utils.DrawText3D(robbery.inHouse.exit.xyz + vector3(0.0,0.0,1.0), "Press [~r~E~s~] to leave.",1,7)
        else
      ...
      if robbery.plyData.job.name ~= Config.PoliceJob then
        ...
        if closestDist < Config.ActDist then
          if Config.UseDraw3D then
            ...
            ESX.Game.Utils.DrawText3D(pos, "Press [~r~E~s~] to try loot the "..robbery.currentInterior.loot[closest].tab..".",1,7)
          else
