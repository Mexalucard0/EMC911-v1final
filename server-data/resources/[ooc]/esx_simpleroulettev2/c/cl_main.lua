...
Citizen.CreateThread(function()
	...
	local coords = GetEntityCoords(PlayerPedId())
		if (GetDistanceBetweenCoords(coords, coordx, coordy, coordz, true) < 5) then
			sleep = false
			ESX.ShowHelpNotification('按 ~INPUT_PICKUP~ 开始玩 ~g~轮盘')
...
function bettaus22()
	maxLength = 5
	AddTextEntry('FMMC_KEY_TIP8', "压上你的 ~y~赌注")
	...
	local elements = {
		{ label = '<span style="color:red;">红色</span>', action = 'red' },
		{ label = '<span style="color:black;">黑色</span>', action = 'black' },
		{ label = '<span style="color:limegreen;">绿色</span>', action = 'green' },
	}
...
function DrawText3D(x,y,z, ruletti)
  ...
  if onScreen then
		...
		SetTextEntry("STRING")
		SetTextCentre(1)
		AddTextComponentString('~w~小球正在 ~g~滚动:~n~~r~ ' ..(ruletti[math.random(1, #ruletti)])..'.')
		DrawText(_x,_y)
    end
end
