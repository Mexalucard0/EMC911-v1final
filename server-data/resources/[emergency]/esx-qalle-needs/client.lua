...
function OpenNeedsMenu()
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'esx-qalle-needsmenu',
        {
            ...
            align    = 'top',
            elements = { 
                { label = 'Pipi', value = 'pee' },
                { label = 'Caca', value = 'poop' }
            }
        },
    function(data, menu)
...
RegisterNetEvent('bnj:pipi')
AddEventHandler('bnj:pipi', function(entity)
            if Config.StatusBars then
                TriggerEvent('esx_status:getStatus', 'pee', function(status)
                    if status.val < 200000 then
                        TriggerServerEvent('esx-qalle-needs:add', 'pee', 200000)
                        local hashSkin = GetHashKey("mp_m_freemode_01")
...
RegisterNetEvent('bnj:caca')
AddEventHandler('bnj:caca', function(entity)
            if Config.StatusBars then
                TriggerEvent('esx_status:getStatus', 'shit', function(status)
                    if status.val < 100 then
                        TriggerServerEvent('esx-qalle-needs:add', 'shit', 100)
                        TriggerServerEvent('esx-qalle-needs:sync', GetPlayerServerId(PlayerId()), 'poop')
                    else
                        ESX.ShowNotification('You don\'t have to poop')
                    end
