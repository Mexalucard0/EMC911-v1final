...
RegisterNetEvent('bnj:tattoXX')
AddEventHandler('bnj:tattoXX', function(coord)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local object = xPlayer.getInventoryItem('ticket').count
    if object >= 30 then
        xPlayer.removeInventoryItem('ticket', 30)
        TriggerClientEvent('bnj:tattoXXX', source)
    else
        TriggerClientEvent('esx:showNotification', source, Config.Text[Language].SRageNotEnoughTickets)
    end
end)

RegisterNetEvent('bnj:eat')
AddEventHandler('bnj:eat', function(coord)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local object = xPlayer.getInventoryItem('plato').count
    if object >= 1 then 
        TriggerClientEvent('bnj:eat55', source)
    else
        TriggerClientEvent('esx:showNotification', source, Config.Text[Language].cantineneed)
    end
end)

RegisterNetEvent('bnj:eat2')
AddEventHandler('bnj:eat2', function(coord)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local object = xPlayer.getInventoryItem('plato').count
    if object >= 1 then
     xPlayer.removeInventoryItem('plato', 1)   
...
AddEventHandler('bnj:hasItem', function(coord)
    ...
    if xPlayer.getInventoryItem('ticket').count >= amount then
        ...
        TriggerClientEvent('esx:showNotification', source, Config.Text[Language].SRageUIFree)
    else
        TriggerClientEvent('esx:showNotification', source, Config.Text[Language].SRageUIWork)
    end
end)
...
AddEventHandler('bnj:GivePrsnPain', function()
    ...
    if object >= 20 then
        ...
        xPlayer.addInventoryItem('plato', 1)
        TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageBuyBread)
    else
        TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageNotEnoughTickets)
    end
end)
...
AddEventHandler('bnj:GivePrsnEau', function()
    ...
    if object >= 20 then
        ...
        TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageBuyWater)
    else
        TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageNotEnoughTickets)
    end
end)
...
AddEventHandler('bnj:GivePrsnCouteau', function(name, price)
    ...
    if xPlayer.hasWeapon(name) then
        TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageWeapnAlreadyOwned)
    else
        if object >= price then
            xPlayer.removeInventoryItem('ticket', price)
            xPlayer.addWeapon(name, 1)
            TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageBuyKnife)
        else
            TriggerClientEvent('esx:showNotification', _source, Config.Text[Language].SRageNotEnoughTickets)
        end 
    end
...
RegisterNetEvent('bnj:barbermenu25')
AddEventHandler('bnj:barbermenu25', function(coord)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local object = xPlayer.getInventoryItem('ticket').count
    if object >= 30 then
        xPlayer.removeInventoryItem('ticket', 30)
        TriggerClientEvent('bnj:barbermenu29', source)
    else
        TriggerClientEvent('esx:showNotification', source, Config.Text[Language].SRageNotEnoughTickets)
    end
end)
...
AddEventHandler('bnj:collectionner', function()
  ...
  --  if object < 40 then
    --if xPlayer.job.name == 'recycleur' then
        ...
        TriggerClientEvent('esx:showNotification', _source, nombre .. Config.Text[Language].SRageCollected)
   -- else
