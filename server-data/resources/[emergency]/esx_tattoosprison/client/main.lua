...
AddEventHandler('skinchanger:modelLoaded', function()
	ESX.TriggerServerCallback('esx_tattoosprison:requestPlayerTattoos', function(tattooList)
		if tattooList then
		...
		if data.current.value then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_shop_categories', {
				...
				if data2.current.value ~= nil then
					...
					ESX.TriggerServerCallback('esx_tattoosprison:purchaseTattoo', function(success)
						if success then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_tattoosprison:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_tattoosprison:hasExitedMarker', LastZone)
		end
	end
end)

AddEventHandler('esx_tattoosprison:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_tattoosprison:hasExitedMarker', function(zone)
	CurrentAction = nil
...
RegisterNetEvent('bnj:tattoX')
AddEventHandler('bnj:tattoX', function()
OpenShopMenu()
end)